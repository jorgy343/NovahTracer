...
bool Network::IsMessageReady() const
{
...
int Network::Send(const char* buffer, int bufferSizeInBytes) const
{
    auto result = send(_connectedSocket, buffer, bufferSizeInBytes, 0);
    auto t = WSAGetLastError();

    return result;
}

void Network::SendChunkRequestedMessage() const
{
    int buffer[2];
...
std::unique_ptr<const char[]> Network::GetMessage()
{
